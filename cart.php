            ...
            <?php if ($shoppingCart->hasProducts()): ?>
                ...
                <?php foreach ($shoppingCart->getProducts() as $index => $product): ?>
                    <div class="shopping-cart__item">
                        ...
                        <span class="price"><h4><?php echo $product->getPrice() ?> </h4></span>
                        <a class="remove-all-button" href="cart.php?action=remove_item&item_index=<?php echo $index?>">verwijderen</a>
                    </div>
                <?php endforeach; ?>
                <strong>Totaalbedrag:

                <?php $sum = 0;
                 foreach ($shoppingCart->getProducts() as $index => $product): ?>
                  <?php $sum += $product->getPrice(); ?>
                <?php endforeach;
                  echo "€";
                  echo $sum;?>
               ...
               <a href="cart.php?action=remove_all_item" class="remove-all-button">alles verwijderen</a>
            <?php else: ?>
